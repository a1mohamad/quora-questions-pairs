...
class AvgMaxPool(nn.Module):
    ...
    def forward(self, x, mask):
        raw_lengths = mask.sum(dim=1, keepdim=True)
        ...
        empty_rows = raw_lengths.squeeze(1) == 0
...
class QuoraSiameseClassifier(nn.Module):
    ...
    def _encode(self, question):
        ...
        pool_mask = self._create_mask(question)
        attention_mask = pool_mask
        if self.stop_mask is not None:
            token_stop_mask = self.stop_mask[question]
            attention_mask = pool_mask * token_stop_mask.float()
        out, _ = self.enhc_lstm(emb)
        out = self.enhc_lstm_norm(out)
        return out, attention_mask, pool_mask

    def forward(self, q1, q2):
        out1, attn_mask1, pool_mask1 = self._encode(q1)
        out2, attn_mask2, pool_mask2 = self._encode(q2)
        
        cross1 = self.cross_attention(query=out1, key_value=out2, mask_kv=attn_mask2)
        cross2 = self.cross_attention(query=out2, key_value=out1, mask_kv=attn_mask1)
        ...
        h1 = self.pool(comp1, pool_mask1)
        h2 = self.pool(comp2, pool_mask2)
